...
# If notifications are already present, dismiss them and exit
# For Dunst, use dunstctl instead of makoctl
if [[ -n "$(dunstctl count displayed)" && "$(dunstctl count displayed)" -gt 0 ]]; then
  dunstctl close-all
  exit 0
fi
