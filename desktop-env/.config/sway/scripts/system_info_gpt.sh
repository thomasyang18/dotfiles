...
ncal_output="$(ncal -b)"
# Remove any ANSI escape sequences from ncal’s output.
ncal_output=$(echo "$ncal_output" | sed -r 's/\x1B\[[0-9;]*[mK]//g')
# Highlight the current day (determined by day-of-month) in gold.
current_day=$(date +%-d)
ncal_output=$(echo "$ncal_output" | sed -E "s/(\b|[[:space:]])($current_day)(\b)/\1<span foreground=\"#ffd700\" weight=\"bold\">\2<\/span>\3/g")
# Wrap the calendar output in white.
ncal_output="<span foreground=\"#ffffff\">$ncal_output</span>"
...
# Get the current workspace name and wrap it in orange.
