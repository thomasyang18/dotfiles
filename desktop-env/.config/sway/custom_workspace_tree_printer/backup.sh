...
jq --slurpfile config "$config_file" -r '
  ...
  ("<span foreground=\"" + pango_colors["white"] + "\">Workspace: " + $ws.name + "</span>"),
  ...
  ($ws | recurse(.nodes[]?) | select(.type == "con" and .name != null) | 
    if .app_id then
      ...
      if $config[0][.window_properties.class] then
        "  <span foreground=\"" + (pango_colors[$config[0][.window_properties.class]]) + "\">" + .name + "</span>"
      else
        "  " + .name + ""
      end
    else
  ...
  ($ws.floating_nodes[]? | select(.type == "con" and .name != null) |
    if .app_id then
      ...
      if $config[0][.window_properties.class] then
        "  <span foreground=\"" + (pango_colors[$config[0][.window_properties.class]]) + "\">" + .name + "</span> (floating)"
      else
        "  " + .name + " (floating)"
      end
    else
